<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['create_order'])) {
            ...
            // 1. Fetch selected pizza's price and name
            $pizza_res  = mysqli_query($conn, "SELECT name, price FROM pizzas WHERE id = $pizza_id");
            $pizza_row  = mysqli_fetch_assoc($pizza_res);
            $pizza_name = $pizza_row['name'];
            $pizza_price = (float) $pizza_row['price'];

            // 2. Loop through selected toppings, fetch prices, build label
            $toppings_total = 0;
            $topping_names  = [];
            foreach ($toppings as $t_id) {
                $t_id = (int) $t_id;
                $t_res = mysqli_query($conn, "SELECT name, price FROM toppings WHERE id = $t_id");
                $t_row = mysqli_fetch_assoc($t_res);
                $toppings_total += (float) $t_row['price'];
                $topping_names[] = $t_row['name'];
            }

            // 3. Calculate grand total
            $grand_total     = ($pizza_price + $toppings_total) * $qty;
            $toppings_label  = implode(', ', $topping_names);

            // 4. INSERT order
            $toppings_safe = mysqli_real_escape_string($conn, $toppings_label);
            $pizza_safe    = mysqli_real_escape_string($conn, $pizza_name);
            mysqli_query($conn,
                "INSERT INTO orders (customer, pizza, toppings, qty, total, status)
                 VALUES ('$customer', '$pizza_safe', '$toppings_safe', $qty, $grand_total, 'Pending')"
            );
            header("Location: " . $_SERVER['PHP_SELF']); exit;
        }
        ...
        <div class="grid-layout">
            ...
            <div class="card">
                ...
                <table>
                    <tbody>
                        <?php
                            // TODO 3: Read from 'pizzas' table
                            $result = mysqli_query($conn, "SELECT * FROM pizzas ORDER BY id ASC");
            ...
            <div class="card">
                ...
                <table>
                    <tbody>
                        <?php
                            // TODO 4: Read from 'toppings' table
                            $result = mysqli_query($conn, "SELECT * FROM toppings ORDER BY id ASC");
                            while ($row = mysqli_fetch_assoc($result)) {
                                $id    = $row['id'];
                                $name  = htmlspecialchars($row['name']);
                                $price = htmlspecialchars($row['price']);
                    ...
                    <tbody>
                        <?php
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $details = "{$qty}x {$pizza}";
                                if (!empty($toppings)) {
                                    $details .= " with {$toppings}";
                                }

                                // Only show complete button if Pending
                                $complete_btn = '';
                                if ($status === 'Pending') {
                                    $complete_btn = "
